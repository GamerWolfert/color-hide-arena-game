...
func _create_brick_sections() -> void:
	...
	for row in range(9):
		...
		for column in range(15):
			...
			var brick_position := Vector3(-7.55 + column * 1.08 + offset, 0.55 + row * 0.47, -6.0)
			_box("Brick_%d_%d" % [row, column], Vector3(1.0, 0.38, 0.10), brick_position, _material(color, 0.05, 0.92))
...
func _create_ring(position_value: Vector3, material: StandardMaterial3D) -> void:
	var ring := TorusMesh.new()
	ring.inner_radius = 0.72
	ring.outer_radius = 1.08
	var mesh := _mesh_node("PurpleRing", ring, material, position_value)
	mesh.rotation_degrees = Vector3(0.0, 18.0, 12.0)

func _create_camo_object(position_value: Vector3, material: StandardMaterial3D) -> void:
	_cylinder("CamoPot", 0.52, 0.43, 0.72, position_value + Vector3(0.0, 0.36, 0.0), _material(Color(0.12, 0.13, 0.18), 0.52, 0.42))
	for index in range(5):
		var angle := float(index) * TAU / 5.0
		_sphere("CamoLeaf_%d" % index, 0.32, position_value + Vector3(cos(angle) * 0.34, 1.15 + sin(angle * 2.0) * 0.12, sin(angle) * 0.26), material)
